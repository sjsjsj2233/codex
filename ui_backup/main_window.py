...
class NetworkAutomationApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.diagnostic_tabs.setTabPosition(QTabWidget.South)
        ...
        self.report_tabs.setTabPosition(QTabWidget.South)
        ...
        self.tools_tabs.setTabPosition(QTabWidget.South)
        ...
        self.tools_tabs.addTab(self.log_analyzer_tab.as_widget(), self.translate("로그 분석"))
        ...
        self.tabs.addTab(self.tools_tabs, self.translate("도구"))  # ✅ 위치 이동!
    ...
    def apply_styles(self):
            ...
            self.setStyleSheet("""
                QMainWindow {
                    background-color: #353535;  /* 얕은 회색으로 변경 */
                }
                QWidget {
                    background-color: #353535;  /* 얕은 회색으로 변경 */
                    color: #E0E0E0;
                    font-size: 9pt;  /* 글자 크기 감소 */
                }
                QTabWidget::pane {
                ...
                QMenu::item {
                    padding: 4px 25px 4px 20px;  /* 패딩 감소 */
                }
                QMenu::item:selected {
                    background-color: #4B6EAF;
                    color: white;
                ...
                QComboBox {
                    ...
                    padding: 4px;  /* 패딩 감소 */
                    color: #E0E0E0;
                    min-height: 20px;  /* 높이 감소 */
                }
                QComboBox::drop-down {
                    border: none;
                    width: 20px;  /* 드롭다운 버튼 너비 감소 */
                }
                QSpinBox {
                    background-color: #353535;
                    border: 1px solid #555555;
    ...
    def start_execution(self):
        ...
        password = self.network_tab.password_input.text().strip()
        enable_password = None
        if self.network_tab.enable_checkbox.isChecked():
            enable_password = self.network_tab.enable_password_input.text().strip()
        save_path = self.network_tab.save_path_input.text().strip()
